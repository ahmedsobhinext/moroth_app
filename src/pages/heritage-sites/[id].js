...
const PlaceDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchPlaces = async () => {
      try {
        const docRef = doc(db, 'heritageSites', id);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white" dir="rtl">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <Link href="/heritage-places" className="mb-8 inline-block group">
          <button className="flex items-center space-x-2 bg-amber-100 hover:bg-amber-200 px-6 py-3 rounded-full transition-all duration-300">
            ...
            <span className="text-amber-900 font-IBMPlexSansArabic text-lg">العودة إلى الأماكن التراثية</span>
          </button>
        </Link>
            ...
            <div className="space-y-8">
              ...
              {places.description && (
                <div className="mt-10 border-t-2 border-amber-100 pt-10">
                  <h2 className="text-3xl font-bold text-amber-900 mb-6 font-IBMPlexSansArabic">تفاصيل المكان</h2>
        ...
        <div className="mt-12 text-center">
          <Link href="/heritage-places" className="inline-block bg-amber-900 text-white px-12 py-4 rounded-full 
            hover:bg-amber-800 hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-xl
            font-IBMPlexSansArabic text-xl">
            عرض جميع الأماكن التراثية
          </Link>
        </div>
      </div>
    </div>
